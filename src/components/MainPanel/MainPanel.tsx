...
interface PreparedTimelineAudio {
  cue: TimelineCue;
  // Chunks generated by TTS but not yet pushed to the streaming track. The tick
  // drains this on each frame so a long cue plays as it is synthesized instead of
  // waiting for the whole sentence (streaming TTS enqueue).
  chunks: Int16Array[];
  // Whether the first chunk has already been pushed to the streaming track.
  started: boolean;
  // Whether generateChinese resolved (no more chunks will be appended).
...
const MainPanel: React.FC<MainPanelProps> = () => {
  ...
  const startTimelineConversation = useCallback(async () => {
    ...
    try {
      ...
      const { tabId, ...response } = await requestCaptions();
      // Pin every later video-time poll to the exact tab the captions came from,
      // so the tick never re-queries the active tab (finding: redundant per-frame
      // tabs.query + URL validation, and the wrong-tab risk if the user switches).
      timelineVideoTabIdRef.current = tabId;
      ...
      const prepareCueAudio = async (cue: TimelineCue) => {
        ...
        try {
          await tts.generateChinese(cue.translatedText, (chunk) => {
            ...
            // creating the prepared entry on the first chunk so the tick can start
            // playback before the whole sentence finishes. A cue that the tick
            // already retired (queued via drop) must not be resurrected here.
            if (!isTimelineSessionActive() || timelineQueuedCueIdsRef.current.has(cue.id)) return;
            const existing = timelinePreparedAudioRef.current.get(cue.id);
            if (existing) {
              existing.chunks.push(chunk.samples);
            } else {
              timelinePreparedAudioRef.current.set(cue.id, {
                cue,
      ...
      const tick = async (): Promise<void> => {
        ...
        for (const [cueId, prepared] of timelinePreparedAudioRef.current) {
          ...
          // remaining chunks finish draining; cutting it here would chop the
          // sentence mid-word.
          if (!prepared.started && prepared.cue.endMs <= currentTimeMs) {
        ...
        if (!videoTime.paused) {
          // Drain a prepared cue's buffered chunks onto the streaming track,
          // emptying the pending buffer so the next tick only pushes newly
          // synthesized chunks (no double-enqueue).
          const drainPreparedChunks = (prepared: PreparedTimelineAudio) => {
            const pending = prepared.chunks.splice(0, prepared.chunks.length);
            for (const chunk of pending) {
              if (!isTimelineSessionActive()) return;
              audioServiceRef.current?.addAudioData(chunk, timelineTrackId, true, {
                itemId: prepared.cue.id,
                cueId: prepared.cue.id,
                startMs: prepared.cue.startMs,
                endMs: prepared.cue.endMs,
                sourceText: prepared.cue.sourceText,
                translatedText: prepared.cue.translatedText,
              });
            }
          };

          // Apply the streaming state machine to one cue: start playback as soon
          // as the first chunk exists, keep appending later chunks to the same
          // track, retire the cue once its tail has drained, and drop a cue the
          // scheduler skipped before it ever started.
          const applyStreamingAction = (
            cue: TimelineCue,
            decision: ReturnType<typeof classifyCuePlayback>,
            prepared: PreparedTimelineAudio | undefined,
          ) => {
            const action = decideStreamingCueAction(decision, {
              started: prepared?.started ?? false,
              generationDone: prepared?.generationDone ?? false,
              pendingChunkCount: prepared?.chunks.length ?? 0,
            });
            switch (action) {
              case 'start':
                if (!prepared) return;
                prepared.started = true;
                drainPreparedChunks(prepared);
                break;
              case 'append':
                if (!prepared) return;
                drainPreparedChunks(prepared);
          ...
          for (const cue of cueWindow) {
            if (!isTimelineSessionActive()) return;
            handledCueIds.add(cue.id);
            // A cue already retired (started+drained, or dropped) must not be
            // touched again — its entry is gone and it is in the queued set.
            if (timelineQueuedCueIdsRef.current.has(cue.id)) continue;
            ...
            applyStreamingAction(cue, decision, timelinePreparedAudioRef.current.get(cue.id));
          }

          // A cue that already started streaming can scroll out of the prebuffer
          // window (endMs <= now) while its tail is still being synthesized.
          // getCueWindow no longer returns it, so drain/finish it here. A started
          // cue is never dropped, so the 'skip' decision only ever appends or finishes.
          for (const [cueId, prepared] of timelinePreparedAudioRef.current) {
            if (!isTimelineSessionActive()) return;
            if (!prepared.started || handledCueIds.has(cueId)) continue;
            applyStreamingAction(prepared.cue, 'skip', prepared);
          }
        }
